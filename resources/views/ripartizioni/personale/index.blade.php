...
<script>
  async function loadTableData() {
    ...
    Object.keys(labels).forEach(k => {
      convIdByKey[k] = parseInt(k.replace(/^c/, '')) || 0;
    });
    ...
    const dt = $table.DataTable({
      ...
      footerCallback: async function() {
        ...
        const syncCells = ($row) => {
          ...
          if (diff > 0)
            for (let i = 0; i < diff; i++) $row.append('<td></td>');
          if (diff < 0)
            for (let i = 0; i < -diff; i++) $row.children('td').last().remove();
        };
        syncCells($totRow);
        syncCells($persRow);
  ...
  function setupCustomSelect(formId, inputId, dropdownId, toggleBtnId, hiddenId) {
    ...
    document.addEventListener('click', e => {
      if (!form.contains(e.target)) hideDd();
    });
  }
