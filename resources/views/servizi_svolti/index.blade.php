...
$(async function () {
  ...
  const staticColumns = [
    { key: 'is_totale', label: '', hidden: true }, // per forzare riga TOTALE in cima
  ...
  convenzioni.forEach(conv => {
    ...
    columns.push({ data: `${conv}_n`, defaultContent: 0 });
    nServiziColumnIndexes.push(visibleIndex);
    visibleIndex++;
    columns.push({ data: `${conv}_percent`, defaultContent: 0 });
    visibleIndex++;
  });
  ...
  columns.push({
    ...
    render: function(row) {
      ...
      return `
        <a href="/servizi-svolti/${row.idAutomezzo}" class="btn btn-sm btn-info me-1">
          <i class="fas fa-eye"></i>
        </a>
        <a href="/servizi-svolti/${row.idAutomezzo}/edit" class="btn btn-sm btn-warning me-1">
  ...
  table.DataTable({
    ...
    columns,
    columnDefs: [
      { targets: 0, visible: false, searchable: false }, // colonna is_totale
      { targets: nServiziColumnIndexes, className: 'kmTh' }
    ],
    order: [[0, 'asc']],      // riga TOTALE (is_totale)
    orderFixed: [[0, 'asc']], // forzata fissa
    responsive: true,
    language: {
