...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // DataTable (6 colonne totali)
  if ($('#convenzioniTable tbody tr').length > 0 &&
      $('#convenzioniTable thead th').length === 6) {
  ...
  document.getElementById('btn-duplica-si')?.addEventListener('click', async function() {
    const btn = this;
    btn.disabled = true; btn.innerText = 'Duplicazione…';
    ...
    if (res.ok) location.reload();
      ...
      btn.disabled = false; btn.innerText = 'Sì';
    }
  });
...
setupCustomSelect("assocFilterForm","assocFilterInput","assocFilterDropdown","assocFilterToggleBtn","assocFilterHidden");
</script>
...
(function () {
  ...
  setTimeout(() => {
    ...
    flash.offsetHeight; // force repaint
